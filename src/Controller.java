    ...
    public void run() {
        ...
        while (running) {
            ...
            while(deltaUPS >= 1) {
                ...
                for(int x=0; x < model.getMap().length; x++) {
                    for(int y=0; y < model.getMap()[x].length; y++) {
                        boolean isAlive = model.getMap()[x][y];
                        if(isAlive) {
                            model.checkSurroundings(x, y);
                            viewNative.draw(model.getShapes(x, y));
                        }
                    }
                }
